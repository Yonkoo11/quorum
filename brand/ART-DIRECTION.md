...
- Two circles of radius `r`, centres `1.06 r` apart (the site's proportion, the narrower of the three in use). This keeps the vesica taller than it is wide, so it reads as an overlap of two things, not as a leaf or an eye. Vesica width = `0.94 r`, height = `1.69 r`.
...
4. **Favicon** (`favicon.svg`, `favicon.ico` 16/32/48): the mark on a square with no radius. At 16 px the stroke is thickened to `0.30 r` and the mark fills 94% of the box, because a 16 px tab has no room for margin. Measured on rendered pixels (rounds/round1/zoom-favicon.png): 0.20 r goes grey, 0.34 r crushes the green to two pixels. The SVG favicon switches to the dark tile under `prefers-color-scheme: dark`; the .ico is the paper tile.
